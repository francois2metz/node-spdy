...
var Server = core.Server = function(options, requestListener) {
  ...
  net.Server.call(this, options, function(c) {
    handleSpdy(that, c, connectionSettings);
  });
...
function handleSpdy(server, c, connectionSettings) {
  c.zlib = createZLib();
  ...
  parser.on('cframe', function(cframe) {
    if (cframe.headers.type == enums.SYN_STREAM) {
      ...
      streams[cframe.data.streamID] = undefined;
        ...
        if ((cframe.data % 2) == 1) return;
        c.write(createPingFrame(c.zlib, cframe.data));
    }
  });
 ...
 * Class @constructor
 */
var TlsServer = core.TlsServer = function(options, requestListener) {
  var that = this,
      connectionSettings = options.connectionSettings || {
        SETTINGS_MAX_CONCURRENT_STREAMS: 100
      };

  if (!(this instanceof TlsServer)) return new TlsServer(options, requestListener);

  tls.Server.call(this, options, function(c) {
    if (tls.NPN_ENABLED && c.npnProtocol != 'spdy/2' && !options.debug) {
...
util.inherits(TlsServer, tls.Server);

core.createTlsServer = function(options, requestListener) {
  return new TlsServer(options, requestListener);
};
